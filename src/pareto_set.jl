...
function populate_results_csv(R,M,V,S,K, λ_set, conv_mask; save_path="")
    save_path = create_empty_results_csv(save_path)
    np = length(λ_set)
    F = obj.calc_F(R,M)
    for i ∈ 1:np  
        is_con = conv_mask[i]
        λ = λ_set[i]
        append_results_csv(F,M,V,S,K, λ, is_con, save_path)
    end
    return save_path
end

function append_results_csv(F,M,V,S,K, λ, is_con, save_path)
    w, st, os, Fw = calc_results_line(F,M,V,S,K,λ)
    open(save_path, "a") do io
        write(io, "$(λ)|$(is_con)|$(w)|$(Fw[1])|$(Fw[2])|$(Fw[3])|$(Fw[4])|$(st)|$(os)\n")
    end
end

function calc_results_line(F,M,V,S,K,λ)
    F_λ_opt_ = obj.get_F_λ_opt(M, V, S, K, λ)
...
read_results_csv(csv_path) = clean_results_csv(CSV.read(csv_path, DataFrame, delim="|"))
